...
public class ReactorTest {
    ...
    public void publishOn() throws Exception {
        WebTestClient.RouterFunctionSpec routerFunctionSpec = WebTestClient.bindToRouterFunction(new ProductRouter().productRoute(new ProductHandler(null, null)));
        routerFunctionSpec.build().get().uri("/products/hello").exchange().expectStatus().is5xxServerError();
        ...
        Flux.range(1, 3)
                ...
                .subscribeOn(Schedulers.newSingle("sub two"))//no momento de subscription,
                    // afeta toda a cadeia onde os dados serao emitidos(upstream) e executados, apenas o primeiro eh considerado
